...
export const ShopFilters = () => {
  ...
  const onClearFilters = () => {
    ...
    setSelectedBrands([]);
    // ? Al no poner searchParams como argumento, no se inicializa, por lo que igualmente borra todo
    // ? Solo bastaria definir uno nuevo, o en todo caso inicializarlo y defnir que parametros se quiere quitar
  ...
  return (
    <div className="space-y-6 px-4 lg:px-0">
          ...
          {!categories.data ? (
            ...
            categories.data.map((category) => (
              <div
                ...
                onClick={() => handleCategoriesChanged(category.categoria)}
              >
                <Checkbox
      ...
      <Card>
        ...
        {!priceRange.data ? (
          <h1 className="text-sm pl-4 text-red-900">
            No hay rango de precios para mostrar
          </h1>
        ) : (
          ...
          {!brands.data ? (
            ...
            brands.data.map((brand) => (
              <div
                key={brand}
                className="flex items-center space-x-2"
                onClick={() => handleBrandChanged(brand)}
              >
                <Checkbox
                  // id={`brand-${brand}`}
      ...
      <div className="flex flex-col items-center gap-3">
        <Button className="w-full" onClick={handleFiltersApplied}>
          Aplicar filtros
        </Button>
        {/* {hasActiveFilters && ( */}
        <Button
